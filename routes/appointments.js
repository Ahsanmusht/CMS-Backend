...
async function prepareEmailRecipients(companyEmailConfig, clientEmail) {
  ...
  // Add company emails
  if (companyEmailConfig && companyEmailConfig.to_emails) {
    try {
 ...
 * /appointments:
 *   post:
 *     summary: Create new appointment with advanced validation
 *     tags: [Appointments]
 *     security:
 *       - bearerAuth: []
 *     requestBody:
 ...
 *         description: Unauthorized
 *       409:
 *         description: Conflict - Duplicate appointment or email
 *       500:
 *         description: Server error
 */
router.post("/", validate(schemas.appointment), async (req, res, next) => {
  ...
  try {
    ...
    // Verify service exists, belongs to company, and is active
    const [serviceCheck] = await connection.execute(
      `SELECT s.id, s.name, s.is_active, s.duration_minutes
       FROM services s 
    ...
    // Verify service variant if provided
    let serviceVariant = null;
    if (service_variant_id) {
      ...
      //   await connection.rollback();
      //   return res.status(400).json({
      //     message: 'Selected service variant is currently unavailable',
      //     error_code: 'SERVICE_VARIANT_INACTIVE'
      //   });
      // }
    }

    // ============ PHASE 4: USER MANAGEMENT WITH COMPANY-SPECIFIC EMAIL UNIQUENESS ============

    let finalUserId = null;
    let shouldSendCompleteProfileEmail = false;
    let newUser = null;
    let existingUserNeedsProfileCompletion = false;
    let userFound = false;
    ...
    if (!isValidUserId) {
      ...
      const [existingUserCheck] = await connection.execute(
        `SELECT id, email, password_hash, first_name, last_name, phone, is_active 
         FROM users 
         WHERE LOWER(email) = ? AND company_id = ?`,
        [cleanEmail, company_id]
      );

      if (existingUserCheck.length > 0) {
        // User exists for this company - use existing user ID
        ...
        const [userResult] = await connection.execute(
          `INSERT INTO users (company_id, first_name, last_name, email, is_active, created_at, updated_at) 
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          [
    ...
    // ============ PHASE 5: APPOINTMENT CONFLICT CHECKS ============

    // Check for appointment conflicts (same user, same day, active status)
    const [conflictCheck] = await connection.execute(
      `SELECT id, date, status, TIME(date) as appointment_time
       FROM appointments 
       WHERE user_id = ? AND DATE(date) = DATE(?) AND status NOT IN ('cancelled', 'completed', 'no_show')`,
      [finalUserId, date]
    );

    if (conflictCheck.length > 0) {
      await connection.rollback();
      const conflictDate = new Date(conflictCheck[0].date).toLocaleDateString();
    ...
    // ============ PHASE 7: FETCH COMPLETE APPOINTMENT DATA ============
    ...
    // Send profile completion email if needed
    if (shouldSendCompleteProfileEmail) {
      try {
        const { token, expiresAt } = generateCompleteProfileToken();

        // Store token in database
        await connection.execute(
          `INSERT INTO user_profile_tokens (user_id, token, expires_at, created_at, is_used) 
           VALUES (?, ?, ?, ?, 0)`,
          [finalUserId, token, expiresAt, new Date()]
        );

        // Prepare user data for profile completion email
    ...
    // Enhanced response with comprehensive appointment details
    res.status(201).json({
      success: true,
