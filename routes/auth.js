...
const { validate, schemas, validatePasswordDetailed } = require('../middleware/validation');
  ...
  try {
    ...
    try {
      ...
      if (!company.allow_user_registration) {
        await db.execute('ROLLBACK');
        return res.status(403).json({
          message: 'User registration is not allowed for this company. Contact your administrator.',
      ...
      if (company.max_users) {
        ...
        if (company.max_users !== null && userCount[0].count >= company.max_users) {
          await db.execute('ROLLBACK');
          return res.status(403).json({
            message: 'Company has reached maximum user limit. Contact your administrator.',
      ...
      console.log(`New user registered: ${normalizedEmail} for company ${company_id} (${company.name}) from IP: ${clientIP}`);
...
router.post('/login', loginRateLimiter, async (req, res, next) => {
  try {
    ...
    if (!isValidPassword) {
      ...
      console.log(`Failed login attempt for email: ${email}, IP: ${req.ip}, UserAgent: ${req.get('User-Agent')}`);
    ...
    console.log(`Successful login: ${email}, Type: ${userType}, IP: ${clientIP}`);
